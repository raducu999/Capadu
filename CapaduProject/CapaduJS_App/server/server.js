...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) =>{
        ...
        //Check to see if id passed in url corresponds to id of Capadu game in database
        MongoClient.connect(url, function(err, db) {
            if (err) throw err;
            var dbo = db.db("CapaduDB");
            var query = { id:  parseInt(data.id)};
            dbo.collection('CapaduGames').find(query).toArray(function(err, result){
                if(err) throw err;
                
                //A Capadu was found with the id passed in url
    ...
    socket.on('host-join-game', (data) => {
        ...
        if(game){
            ...
            MongoClient.connect(url, function(err, db){
                ...
                dbo.collection("CapaduGames").find(query).toArray(function(err, res) {
                    if (err) throw err;
    ...
    socket.on('requestDbNames', function(tocken){
        
        MongoClient.connect(url, function(err, db){
            if (err) throw err;
    
            var dbo = db.db('CapaduDB');
            dbo.collection("CapaduGames").find().toArray(function(err, res) {
                if (err) throw err;
                //get list of user's capadus
                ...
                getJSON('http://capadu/api/sendcapadus/' + tocken, function(error, response){
                    ...
                    //remove ellements from other users
                    
                    for (var i = res.length - 1; i >= 0; --i) {
                        if (CapadusList[res[i].id] != 1) {
                            res.splice(i,1);
                        }
    ...
    socket.on('newQuiz', function(data){
        MongoClient.connect(url, function(err, db){
            if (err) throw err;
            var dbo = db.db('CapaduDB');
            dbo.collection('CapaduGames').find({}).toArray(function(err, result){
                if(err) throw err;
                ...
                var game = data;
                dbo.collection("CapaduGames").insertOne(game, function(err, res) {
                    if (err) throw err;
                    db.close();
                });
